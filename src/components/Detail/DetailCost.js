...
function DetailCost() {
  ...
  const beneOnClick = (i) => {
    //수급자 유형 버튼 클릭시 하이라이팅, 한번에 한개의 버튼만 선택할 수 있게 바로 전 선택을 저장한 후 다른 버튼 클릭 시 호버 풀어줌.
    if (i.target.id === beneClickid) return;
  ...
  const careOnClick = (i) => {
    // 요양 등급 버튼 클릭시 하이라이팅, 한번에 한개의 버튼만 선택할 수 있게 바로 전 선택을 저장한 후 다른 버튼 클릭 시 호버 풀어줌.
    if (careClickid === i.target.id) return;
  ...
  useEffect(() => {
    if (careClickid !== "") setCareCost(costArr[careClickid]);
  }, [careClickid]);
  useEffect(() => {
    if (beneClickid !== "") setBeneCost(costArr[beneClickid]);
  }, [beneClickid]);
    ...
    <StyledDetailCost>
      ...
      <ResCost>
        <div style={{ borderBottom: "1px dashed #FFFFFF", padding: "4%" }}>
          <ResCostText style={{ marginLeft: "25%" }}>
            {beneCost * careCost}원
          </ResCostText>
        ...
        <div style={{ padding: "4%" }}>
          ...
          >
            <ResCostText>{careCost * 30 * beneCost}원</ResCostText>
          </div>
        </div>
      </ResCost>
    </StyledDetailCost>
